...
prefix = 'sagemaker'
file_name = 'member_periods_v7.parquet'
...
n_months = len(months)
last_valid_pre_start = n_months - 18 # 42
...
# mdf = member_periods.loc[(member_periods.pre_0) & (member_periods.pre_full_0) & (member_periods.member_id == 102)].sort_values('eom')
    ...
    data = np.concatenate((ddos_tc_data[0], state_data, lob_data, group_data, np.array([demographic_data.is_male, demographic_data.age, demographic_data.member_id])), axis=0)
...
def build_period_df(i):
    ...
    final.to_parquet(f'./data/final_wide_df_v7_{i}.parquet')
...
if __name__ == '__main__':
    with Pool() as p:
        ...
        s3_resource.Object(bucket, 'master_wide_df_v7.csv').put(Body=csv_buffer.getvalue())
