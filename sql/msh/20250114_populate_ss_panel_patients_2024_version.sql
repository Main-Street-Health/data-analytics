...
BEGIN
    BEGIN
        ...
        -- Temporary Exit the procedure early if in 2025
        IF now() >= '2025-01-01'::date THEN
            RETURN;
        END IF;
...
--      - Patients with measure med and no open task adr < 20 and max refill date is 5 days from now
        -- 2024-10-17 BP updated 2 to 5 days before
        INSERT
        INTO
            _patients_to_pull (patient_id, reason)
        SELECT DISTINCT
            patient_id
          , 'Pull 5 days before expected next fill date when adr < 20 or pdc < .9: ' || STRING_AGG(pp.measure_key, ', ')
        FROM
            _potential_patients pp
        WHERE
              (pp.adr < 20 OR pp.pdc_to_date < .9)
...
--           AND pp.next_fill_date + '2 days'::INTERVAL <= NOW()
        ...
        WHERE
              pp.has_measure
          AND pp.adr <= 29
          AND next_fill_date <= (select _process_date from _controls) - '2 days'::INTERVAL
          AND NOT has_workflow
          AND NOT is_weird_status
          AND last_panel_created_at < (select _five_days_ago from _controls)
        GROUP BY 1
        ...
        --   - Patients with measure med and no open task and max refill date was 10+ days ago
        -- 2024-07-31 bp changed 5day to 10 days per https://github.com/Main-Street-Health/member-doc/issues/12263
        INSERT
        INTO
            _patients_to_pull (patient_id, reason)
        SELECT DISTINCT
            patient_id
          , 'Pull 10 days after expected next fill date: ' || STRING_AGG(pp.measure_key, ', ')
        FROM
            _potential_patients pp
        WHERE
              pp.has_measure
          AND pp.adr >= 30
          AND next_fill_date <= (select _ten_days_ago from _controls)
          AND NOT has_workflow
          AND NOT is_weird_status
          AND last_panel_created_at < (select _five_days_ago from _controls)
        GROUP BY 1
...
--           AND pp.next_fill_date - NOW()::DATE BETWEEN 63 AND 69 -- upd[2024-07-18] 90-21 = 69. Made it a 6 day range in the off chance it doesn't run on the exact date
        ...
        WHERE
              qpm.is_active
          AND measure_status_key IN
              ('pending_rx_pickup', 'on_hold_first_fill_check', 'closed_pending', 'unable_to_reach', 'refused')
