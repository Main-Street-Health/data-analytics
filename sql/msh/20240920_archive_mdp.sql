...
3220279
SELECT count(inserted_at) FROM analytics.bk_up.bk_md_portal_suspects;
...
select id, s.inbound_file_id, job_run_step_id, s.patient_id, organization_source_patient_id,
                                  first_name, last_name, date_of_birth, icd_10_code, icd_10_code_desc, hcc_category,
                                  hcc_category_desc, reason, source_fact_name, source_supportive_evidence,
                                  source_excerpt, source_date, source_author, source_description, source_location,
                                  compendium_url, last_updated, meta, inserted_at, source_evidence_type,
                                ...
                                from
                                    rawrp.md_portal_suspects s
join junk.md_portal_suspects_max_pat_batch j on j.patient_id = s.organization_source_patient_id and j.inbound_file_id = s.inbound_file_id

ALTER TABLE md_portal_suspects
    OWNER TO postgres;

CREATE INDEX md_portal_suspects_organization_source_patient_id_idx1
    ON md_portal_suspects(organization_source_patient_id);

CREATE INDEX md_portal_suspects_inbound_file_id_idx1
    ON md_portal_suspects(inbound_file_id);

CREATE INDEX md_portal_suspects_inbound_file_id_organization_source_pati_idx
    ON md_portal_suspects(inbound_file_id, organization_source_patient_id)
    WHERE (md_portals_batch_id IS NULL);

GRANT SELECT ON md_portal_suspects TO readonly;

GRANT DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON md_portal_suspects TO readwrite;
Show table preview