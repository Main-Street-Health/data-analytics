    ...
    WHERE
          dp.is_active
    and exists (
        select 1 from fdw_member_doc.supreme_pizza sp
                 where sp.patient_id = dp.source_id
                 and sp.primary_referring_partner_id in (
                      188	-- Burton Creek
                    , 213 --	CCWV - Clarksburg
                    , 225 --	CCWV - Clay
                    , 227 --	CCWV - Big Otter
...
WHERE
  ...
  AND inserted_at::DATE >= '2023-10-05'
...
select etl.dpc_eligibility_to_coop_current_coverage(); -- let sean know theres new data
