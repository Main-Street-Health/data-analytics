CREATE PROCEDURE sp_refresh_dpc_roster_and_attribution()
    LANGUAGE plpgsql
AS
$$
BEGIN
    ...
    WITH
        patient_docs AS ( SELECT
                          ...
                          FROM
                              ...
                              LEFT JOIN fdw_member_doc.msh_physicians mp
                                        ON pc.contact_id = mp.contact_id AND LENGTH(mp.npi::TEXT) = 10 AND
                                           NOT mp.deleted
                          WHERE
                                sp.is_dpc
                            AND pc.is_primary
                          ...
                          FROM
                              fdw_member_doc.supreme_pizza sp2
                              LEFT JOIN fdw_member_doc_stage.patient_physician_location_hierarchy hier
                                        ON hier.patient_id = sp2.patient_id
                              LEFT JOIN fdw_member_doc.msh_physicians mp2
                                        ON hier.msh_physician_id = mp2.id AND LENGTH(mp2.npi::TEXT) = 10 AND
                                           NOT mp2.deleted
                          WHERE
                              sp2.is_dpc )
    ...
    WHERE
        p.mbi IS NOT NULL
    ORDER BY p.patient_id, doc.rank
    ;
...
--     SELECT COUNT(*) FROM _possible_phys;
    ...
    FROM
        _possible_phys pp
        JOIN dpoc_patients dp ON dp.source_id = pp.patient_id
    ORDER BY
        patient_id, rank;
...
------------------------------------------------------------------------------------------------------------------------
-- clean out any not in current attribution _final_pat_pract
    WITH
        archived AS (
            INSERT
                INTO
...
--     CREATE TABLE junk.dpoc_npis_to_run_20230523 AS
    ...
    FROM
        _final_pat_pract fpp;
--     ON CONFLICT DO NOTHING ; -- shouldn't have any conflicts, somethings wrong if we do, blow up
