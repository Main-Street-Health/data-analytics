...
--     select count(*) from _pats;
    ...
    DELETE FROM _pats p WHERE exists(select 1 from _last_export_results ler where ler.mbi = p.mbi and not ler.has_export_error and ler.export_type like '%Coverage%' and ler.last_exported_at >= now() - '2 week'::interval);
...
where sp.primary_referring_partner_id = 213
...
SELECT
    npi
     , case when sp.patient_id is not null then 1 else 2 end order_col
  , COUNT(*) n
FROM
    _assigned_patients ap
left join fdw_member_doc.supreme_pizza sp on sp.patient_mbi = ap.mbi and sp.primary_referring_partner_id = 213
GROUP BY 1,2
ORDER BY order_col,  3 DESC
LIMIT 750 ;

-- 476
SELECT count(distinct sp.patient_id)
FROM
    _providers_to_run ptr
    join _assigned_patients ap on ap.npi = ptr.npi
    join fdw_member_doc.supreme_pizza sp on sp.patient_mbi = ap.mbi and sp.primary_referring_partner_id = 213
;
...
WHERE
  ...
  AND inserted_at::DATE >= '2023-09-11'
