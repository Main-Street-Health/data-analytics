DROP TABLE IF EXISTS _access_med_ar;
CREATE TEMP TABLE _access_med_ar AS
select distinct sp.patient_id
FROM
    fdw_member_doc.msh_referring_partner_organizations rpo
    JOIN fdw_member_doc.referring_partners rp ON rp.organization_id = rpo.id
    JOIN fdw_member_doc.supreme_pizza sp ON sp.primary_referring_partner_id = rp.id
WHERE
      rpo.name = 'Access Medical Clinic'
  AND rp.state = 'AR'
  AND sp.is_dpc
;
...
-- access_med_ar_2022_2023_20230710
SELECT
    p.mbi
...
FROM
    _access_med_ar pats
    join dpoc_patients p on pats.patient_id = p.source_id
...
WHERE
        COALESCE(c.billable_period_start ::DATE, dcl.serviced_date ::DATE, dcl.service_period_start ::DATE)
