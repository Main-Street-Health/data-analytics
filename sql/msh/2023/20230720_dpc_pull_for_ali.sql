...
-- create index on junk.dpc_pull_infusions_20230710(mbi);
-- create index on junk.dpc_pull_infusions_20230710(dos);
-- create index on junk.dpc_pull_infusions_20230710(procedure_code);

DROP TABLE IF EXISTS _claim_hits;
CREATE TEMP TABLE _claim_hits AS
SELECT dc.id
FROM
    junk.dpc_pull_infusions_20230710 j
join dpoc_patients dp ON j.mbi = dp.mbi
join dpoc_claims dc on dc.patient = dp.bene_id
join dpoc_claim_lines dcl ON dc.id = dcl.dpoc_claim_id
where dc.billable_period_start::date = j.dos and dcl.procedure_code = j.procedure_code
;
create index on _claim_hits(id);
...
WHERE
    exists(select 1 from _claim_hits ch where ch.id = c.id);
