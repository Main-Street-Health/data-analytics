    ------------------------------------------------------------------------------------------------------------------------
    /* send NEW chase and attribute file for new patient measures for these projects:
        'anthem_ky', 'anthem_tn', 'bcbst', 'centene_tn', 'humana_al', 'humana_tn', 'uhc_al', 'uhc_ar', 'uhc_tn', 'uhc_wv'
    Only include open measures with source = mco, mco_fall_off
    Doesn't matter if they have a CCA in 2023 or not */
    ------------------------------------------------------------------------------------------------------------------------
    ...
    WHERE
            ptr.name not IN ('anthem_ky', 'anthem_tn', 'bcbst', 'centene_tn', 'humana_al', 'humana_tn', 'uhc_al', 'uhc_ar', 'uhc_tn', 'uhc_wv')
    ...
    -- NEW
    -- Delete out already submitted chases
    DELETE
    FROM
...
--     SELECT count(*) FROM _reveleer_chase_file;
    ...
    ------------------------------------------------------------------------------------------------------------------------
    /* Deal with BCS Gender BS
       If a member is sent to reveleer for BCS they must always be female for all measures
    */
    ------------------------------------------------------------------------------------------------------------------------
