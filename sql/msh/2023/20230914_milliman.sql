...
WHERE
    -- removed burton creek rp 188 per leslie 20230914
      rp.id IN (276, 273, 274, 275, 225, 227)
  -- add 213 on next run
  AND attr.is_om
...
FROM
    ( SELECT
      ...
      JOIN _patients p on p.patient_id = h.patient_id
      WHERE
          h.sure_scripts_panel_id = 5281
      AND not exists (select 1 from _dpc_milliman_rx_claims rx where rx."MemberId" = h.patient_id)
      ) x
;
