...
FROM
    ...
    JOIN dpoc_patients dpat ON dpat.mbi = p.mbi
    ...
    JOIN dpoc_practitioner_group_patients gp ON gp.mbi = p.mbi
    JOIN dpoc_practitioners dp ON gp.npi = dp.npi
    JOIN dpoc_bulk_export_jobs j ON j.dpoc_practitioner_id = dp.id
WHERE
        rp.id IN (
                  274, 273, 275, 276, 117, 192, 26, 32, 30, 194, 193, 138
        )
  AND   attr.is_om
and p.patient_id in ( 1053,17822,118658,197659,119225,3999,7573,194046,29055,193618,197804,319738 )
limit 1000;
...
SELECT
  ...
  , c.coverage                                                     "PlanId"
  , cl.service_period_start                                        "FromDate"
  , cl.service_period_end                                          "ToDate"
  , NULL                                                           "AdmitDate"
  , NULL                                                           "DischDate"
  , drgs.code                                                      "DRG"
