...
--         JOIN public.reveleer_projects ptr ON mpm.payer_id = ptr.payer_id
    ...
    INTO
        _patient_measures (patient_id, chase_id, patient_mbi, payer_name, reveleer_project_id, measure_code,
                           ...
                           must_close_by_date, subscriber_id)
    SELECT DISTINCT
        sp.patient_id
      , NULL::BIGINT chase_id
      , sp.patient_mbi
...
--     create INDEX on reveleer_chases using gin (qm_patient_measure_ids);
    ...
    WHERE
        pm.measure_code IN ('CBP', 'BPD')
    and pbp.systolic is not null
    and pbp.diastolic is not null
...
--       AND measure_value::numeric between 0 and 20
    ORDER BY pm.patient_id, hb.test_date;
    ...
    WHERE
        NOT EXISTS( SELECT
                    ...
                    FROM
                        public.reveleer_chase_file_details cfd
                    WHERE
                          cfd.reveleer_chase_id = af.chase_id
                      AND cfd.reveleer_file_id IS NOT NULL
                      AND cfd.yr = ( SELECT yr FROM _controls ) );
...
------------------------------------------------------------------------------------------------------------------------
    ...
    -- if a REFRESH, remove chases where attributes don't exist, no point in sending chase with no new info
    DELETE
    FROM
        _reveleer_chase_file cf
    WHERE
    ...
    FROM
        ( VALUES
              ...
              ('HBD', 'hba1c_9', 'hbd_hemoglobin_a1c_control_for_patients_with_diabetes'),
...
--           , ROW_NUMBER() OVER ()         row_id
          ...
          , NOW()                        inserted_at
          , NOW()                        updated_at
--           , pqm.mco_patient_measure_id   state_payer_id
          , ( SELECT yr FROM _controls ) yr
          , pm.reveleer_project_id
          , false                        is_new
        FROM
            _patient_measures pm
            JOIN _num_code_mappings ncm ON ncm.measure_key = pm.measure_key;
--             join reveleer_chases rc on pm.re = any(rc.qm_patient_measure_ids)
...
--           , pqm.mco_patient_measure_id   state_payer_id
        ...
        FROM
            fdw_member_doc.qm_patient_measures pqm
            JOIN fdw_member_doc.qm_ref_measures m ON m.key = pqm.measure_key
            JOIN _num_code_mappings ncm ON ncm.measure_key = pqm.measure_key
            join reveleer_chases rc on pqm.id = any(rc.qm_patient_measure_ids)
        WHERE
