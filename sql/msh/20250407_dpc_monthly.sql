...
-- WHERE
--     id = 224003701;
-- truncate adp_worker_leaves RESTART IDENTITY CASCADE ;

SELECT *
FROM
    analytics.oban.oban_crons where opts::text ~* 'unique';
select name, expression, worker, opts, paused, lock_version, inserted_at, updated_at
from oban.oban_crons where name ilike '%ptc%';
