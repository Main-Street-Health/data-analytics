-- create table junk.uhc_supp_file_mbi_error_mapping_20230616 as
-- SELECT DISTINCT on (j.mbi_need_either_member_id_or_mbi)
-- --     j.error
--                j.mbi_need_either_member_id_or_mbi mbi
-- --               , m.source_member_id
-- --               , m.payer_member_id
--               , m.subscriber_id
-- --               , m.mco_id
-- --               , mmi.line_of_business_id
-- --               , mmi.is_primary_across_lobs
-- FROM junk.uhc_supp_file_errors_20230615 j
--          JOIN fdw_member_doc.analytics_patients ap
--               ON ap.medicare_no = j.mbi_need_either_member_id_or_mbi
--          JOIN cb.member_mco_ids mmi
--               ON mmi.member_id = ap.analytics_member_id
--          JOIN cb.member_payer_ids m
--               ON m.member_id = ap.analytics_member_id
--                   AND mmi.is_primary_across_lobs
-- WHERE error ~* 'memberid'
-- order by j.mbi_need_either_member_id_or_mbi, m.created_at desc
-- ;



-- drop TABLE perm.supp_file_uhc_20230616;
CREATE TABLE perm.supp_file_uhc_20230616 AS
WITH
    good_mbi                 AS
        ( SELECT
              mco_id
          ...
          WHERE
              ...
              and not exists(
                  -- exclude failed mbi's, should fallback to ins id
                  select 1 from junk.uhc_supp_file_mbi_error_mapping_20230616 e  where e.mbi = ap.medicare_no
              )
          )
  , encounter_icds           AS ( SELECT
                                  ...
                                  WHERE
                                    ...
                                    AND vn.signed_time >= '2022-03-22' --'{from_date}'
                                    AND i.code not in ( 'M545', 'I714', 'D7582' ) -- not valid for billing per resp file
                                    and (vn.document_date >= '2023-01-01' or i.code not in ('F03911', 'Z7985')) -- too new `23
...
SELECT count(*) FROM perm.supp_file_uhc_20230608;
SELECT count(*) FROM perm.supp_file_uhc_20230616;
SELECT medicare_specialty_code_int FROM perm.supp_file_uhc_20230616;
SELECT place_of_service, count(*) FROM perm.supp_file_uhc_20230616 GROUP BY 1;

SELECT
    j.error
  , j.mbi_need_either_member_id_or_mbi
  , m.mbi
  , m.*
FROM
    junk.uhc_supp_file_errors_20230615 j
    JOIN fdw_member_doc.analytics_patients ap ON ap.medicare_no = j.mbi_need_either_member_id_or_mbi
    JOIN cb.members m ON m.id = ap.analytics_member_id
WHERE
    error ~* 'memberid'
;
